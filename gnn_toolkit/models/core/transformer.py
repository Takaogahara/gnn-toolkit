    ...
    def __init__(self, model_params):
        ...
        for i in range(self.n_layers):
            ...
            if i % self.top_k_every_n == 0:
                self.pooling_layers.append(TopKPooling(
                    embedding_size, ratio=top_k_ratio))
    ...
    def forward(self, x, edge_attr, edge_index, batch):
        ...
        for i in range(self.n_layers):
            ...
            if i % self.top_k_every_n == 0 or i == self.n_layers:
                x, edge_index, edge_attr, batch, _, _ = self.pooling_layers[
                    int(i/self.top_k_every_n)](x, edge_index, edge_attr, batch)
        ...
        if torch.isnan(torch.mean(self.linear3.weight)):
